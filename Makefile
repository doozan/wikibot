...
DUMP_LEMMAS := $(PYENV) $(BUILDDIR)/enwiktionary_wordlist/scripts/dump_lemmas

WIKI2TEXT := $(PYENV) scripts/wiki2text
WIKIGREP := $(PYENV) scripts/wikigrep
WIKISEARCH := $(PYENV) scripts/wikisearch
WIKISORT := $(PYENV) scripts/wikisort
GETLINKS := $(PYENV) scripts/getlinks
GETIGNORE := $(PYENV) scripts/getignore
WIKIFIX := $(PYENV) scripts/wikifix

LIST_DUPLICATE_PASSAGES := $(PYENV) ./list_duplicate_passages.py
LIST_VERBS_MISSING_TYPE := $(PYENV) ./list_verbs_missing_type.py
LIST_MISSING_FORMS := $(PYENV) ./list_missing_forms.py
LIST_MAYBE_FORMS := $(PYENV) ./list_maybe_forms.py
LIST_MISMATCHED_HEADLINES := $(PYENV) ./list_mismatched_headlines.py
LIST_FORMS_WITH_DATA := $(PYENV) ./list_forms_with_data.py
MAKE_SECTION_STATS := $(PYENV) ./make_section_stats.py
LIST_T9N_PROBLEMS := $(PYENV) ./list_t9n_problems.py
LIST_ISMO_ISTA := $(PYENV) ./list_ismo_ista.py
LIST_COORD_TERMS := $(PYENV) ./list_coord_terms.py
LIST_USUALLY_PLURAL := $(PYENV) ./list_usually_plural.py
LIST_SPLIT_VERB_DATA := $(PYENV) ./list_split_verb_data.py
LIST_SECTION_HEADER_ERRORS := $(PYENV) ./list_section_header_errors.py
LIST_SECTION_ORDER_ERRORS := $(PYENV) ./list_section_order_errors.py
LIST_SECTION_LEVEL_ERRORS := $(PYENV) ./list_section_level_errors.py
LIST_DRAE_ERRORS := $(PYENV) ./list_drae_errors.py
LIST_MISSING_DRAE := $(PYENV) ./list_missing_drae.py
LIST_DRAE_MISMATCHED_GENDERS := $(PYENV) ./list_drae_mismatched_genders.py
LIST_ES_FORM_OVERRIDES := $(PYENV) ./list_es_form_overrides.py
LIST_BARE_QUOTES := $(PYENV) ./list_bare_quotes.py
LIST_CONVERT_LIST_TO_COL := $(PYENV) ./list_convert_list_to_col.py
LIST_UNBALANCED_DELIMITERS := $(PYENV) ./list_unbalanced_delimiters.py
LIST_QUOTE_WITH_BARE_PASSAGE := $(PYENV) ./list_quote_with_bare_passage.py
LIST_SENSE_BYLINES := $(PYENV) ./list_sense_byline_errors.py
LIST_BARE_UX := $(PYENV) ./list_bare_ux.py
DUMP_TEMPLATE_DATA := $(PYENV) ./dump_template_data.py
DUMP_MODULE_DATA := $(PYENV) ./dump_module_data.py
LIST_BAD_TEMPLATE_PARAMS := $(PYENV) ./list_bad_template_params.py
COUNT_TEMPLATE_USE := $(PYENV) ./count_template_use.py
MAKE_TEMPLATE_STATS := $(PYENV) ./make_template_stats.py
DUMP_TAXONS := $(PYENV) ./dump_taxons.py
LIST_LOCAL_TAXONS := $(PYENV) ./list_local_taxons.py
LIST_EXTERNAL_TAXONS := $(PYENV) ./list_external_taxons.py
LIST_POSSIBLE_TAXONS := $(PYENV) ./list_possible_taxons.py
LIST_MISSING_TAXLINKS := $(PYENV) ./list_missing_taxlinks.py
LIST_TAXONS_WITH_REDLINKS := $(PYENV) ./list_taxons_with_redlinks.py
DUMP_TEMPLATE_USE := $(PYENV) ./dump_template_use.py
LIST_DEF_TEMPLATE_IN_ETY := $(PYENV) ./list_def_template_in_ety.py
DUMP_CAT := $(PYENV) ./dump_cat.py
LIST_MISSING_HEADERS := $(PYENV) ./list_missing_headers.py

EXTERNAL := ../..
PUT := $(PYENV) $(EXTERNAL)/put.py
FUN_REPLACE := $(PYENV) $(EXTERNAL)/fun_replace.py
...
$(LIST)bad_template_params $(BUILDDIR)/bad_template_calls.json &: $(BUILDDIR)/all-en.enwikt.txt.bz2 $(BUILDDIR)/template_data.json $(BUILDDIR)/redirects.tsv
>   @echo "Running $@..."

>   $(LIST_BAD_TEMPLATE_PARAMS) $(SAVE) --wxt $< --templates $(word 2,$^) --redirects $(word 3,$^) --dump-json $(BUILDDIR)/bad_template_calls.json
>   touch $@
