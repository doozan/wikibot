...
$(LIST)pronunciation_errors: $(BUILDDIR)/all-en.enwikt.txt.bz2
>   @echo "Running $@..."
...
$(FIX)pronunciation:
>   SRC="User:JeffDoozan/lists/pronunciation/fixes"
>   FIX="--fix pronunciation --log-fixes $@.fixes --log-matches $@.matches --config etc/autodooz-fixes.py"
>   MAX=1000
...
slow_lists: $(patsubst %,$(LIST)%, section_header_errors section_level_errors section_order_errors sense_bylines pronunciation unbalanced_delimiters missing_taxlinks t9n_problems convert_list_to_col es_missing_forms def_template_in_ety quote_with_bare_passage bare_ux missing_headers fr_missing_lemmas )
