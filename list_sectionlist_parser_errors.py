...
class WikiSaverByError(BaseHandler):
    ...
    def page_name(self, page_sections, prev):
        if "autofix" in page_sections[0][0].error:
            return "sectionlist/fixes"
        else:
            return "sectionlist/errors"
            #return "sense_bylines/" + page_sections[0][0].error

    def format_entry(self, entry, prev_entry):
        if "autofix" in entry.error:
            return [f": [[{entry.page}]] {entry.section}"]

        if entry.details:
            return [f": [[{entry.page}#{entry.language}|{entry.page}]] {entry.section}\n<pre>{entry.details}</pre>"]
        else:
            return [f": [[{entry.page}#{entry.language}|{entry.page}]] {entry.section}"]
...
class WikiSaverByLanguage(BaseHandler):
    ...
    def sort_items(self, items):
        ...
        # sort by language and then by count of section entries (smallest to largest)
...
#        return page_sections[-1][-1].error.startswith("autofix") and not section_entries[0].error.startswith("autofix")
    ...
    def page_name(self, page_sections, prev):
        return "sectionlist/errors_by_language"
...
class Logger(WikiLogger):
    _paramtype = namedtuple("params", [ "error", "page", "language", "section", "details" ])

logger = Logger()
def log(error, page, section, details):
    lang = section.split(":", 1)[0]
    logger.add(error, page, lang, section, details)
...
def process(args):
    ...
    log = []
    for section in entry.ifilter_sections(matches=lambda x: x.title == "Synonyms"):
        links = section_to_links(section, log)

    return log

def main():
    global fixer
    parser = argparse.ArgumentParser(description="Find unparsable section lists")
