...
TITLE_TEMPLATES = get_aliases("qualifier")
...
SENSE_TEMPLATES = get_aliases("sense", "sense-lite")
...
def get_link(lang_id, section, line):
    """
    Returns a string
    ...
    link = re.sub("<!--\s*-->", "", link, flags=re.DOTALL)

    return link



def get_links(lang_id, section, line):
    ...
    for splitter in [",", ";", "/"]:
        ...
        if len(parts) > 1:
            ...
            links = [get_link(lang_id, section, part) for part in parts if part.strip()]
            if not links:
                continue
    ...
    if line.startswith("{{col"):
        ...
        if str(template.name).strip() not in ["col", "col2", "col3", "col4", "col5", "col6"]:
        ...
        # make sure it's just simple template call without any named params
        if not all(str(p.name).strip().isdigit() for p in template.params):
        ...
        return [str(p.value).strip() for p in template.params if str(p.name).strip() != "1" and str(p.value).strip() ]


    link = get_link(lang_id, section, line)
    if link:
        #print("LINK" ,link)
        return [link]
...
TITLE_TEMPLATES = Q_TEMPLATES
RE_TITLE_TEMPLATES = "|".join(sorted(TITLE_TEMPLATES))
RE_SENSE_TEMPLATES = "|".join(sorted(SENSE_TEMPLATES))
def extract_titles(section, line):

    m = re.match(r"^(\*?\s*)\{\{\s*(" + RE_SENSE_TEMPLATES + r")\s*\|", line)
    if m:
#        print("MATCH", line)
        wikicode = mwparser.parse(line)
        template = next(wikicode.ifilter_templates(), None)
        assert str(template.name).strip() in SENSE_TEMPLATES, f"bad pattern match {line} {m.group(0)}"

        if not all(str(p.name).strip().isdigit() for p in template.params):
            warn("unhandled_sense_params", section, str(template))

        titles = tuple(str(p.value).strip() for p in template.params)

        return titles, line[len(m.group(1))+len(str(template)):]
...
#        print("NO MATCH", line)
        ...
        # Q templates count as titles when followed by a colon as in "Q: foo, bar" or when simply "Q" on its own line
        m = re.match(r"\*?\s*\{\{\s*(?:" + RE_TITLE_TEMPLATES + r")\s*[|]([^\|\]}]+)}}(?:\s*:|\s*$)", line)
        if m:
...
def section_to_links(section, log=None):
    ...
    for line in content_lines:
        ...
        links = get_links(lang_id, section, line)
        #print(links, line)
        if links is None:
            #print("NO LINK", line)
            # unparsable
